...
- **Local integration runs no longer delete each other's workflows** ([#1102](https://github.com/czlonkowski/n8n-mcp/issues/1102)). The instance-wide sweep of leftover test workflows ran from the `afterAll` hook of 17 integration test files while vitest ran those files in parallel, so the first file to finish removed workflows the others were still using; the suite then failed with a 404 in a different file on every run. The sweep now runs once, before the suite, as a vitest `globalSetup` registered only in the integration config (`npm run test:integration` and `npm run test:integration:n8n`; a plain `npm test` never touches the instance). It returns quietly without credentials or against the mock URL, warns once when the instance is unreachable, is bounded to 15 seconds, and skips workflows younger than five minutes; the maintenance script `npm run test:cleanup:orphans` passes an age of zero and keeps deleting fresh leftovers. Deletion tests track the workflows they create so a failed deletion no longer leaks them. The integration config no longer registers `setupFiles` a second time on top of the base config.
- **Docker image builds no longer fail on Dependabot and fork pull requests.** Those runs receive no repository secrets, so the Docker Hub login step failed with "Username and password required" before the image was built. The login now runs only when the secret is present; image pushes were already limited to pushes on `main`.
